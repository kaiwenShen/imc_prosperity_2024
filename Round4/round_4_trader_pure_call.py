...
class Trader:
    ...
    def tongfei_predict_iv(self, ivs):
        ...
        print(f"t stats is: {ols_result['t_stats']}")
    ...
    def tongfei_calculate_fair_price(self, product, state, ordered_position, estimated_traded_lob, latest_coconut_price, predicted_iv):
        ...
        if np.abs(fair_price - mid_price) < 1:
            print(f"At timestamp {state.timestamp}, the price error is within 1")
        # one standard deviation
        if mid_price > fair_price + 1:
            # the price is considered overvalued
            return -1
        elif mid_price < fair_price - 1:
            # the price is considered undervalued
            return 1
        else:
            return 0

    def tongfei_BS_trade(self, product, state, ordered_position, estimated_traded_lob, trade_coef, predicted_iv, current_iv):
        orders_coupon: List[Order] = []
        buy_available_position_coupon, sell_available_position_coupon = self.cal_available_position(product, state, ordered_position)
        best_bid_coupon, best_bid_coupon_amount, best_ask_coupon, best_ask_coupon_amount = self.get_best_bid_ask(product, estimated_traded_lob)
        ...
        if trade_coef == 1:
            ...
            if buy_available_position_coupon > 0:
                ...
                order_coupon, buy_available_position_coupon, estimated_traded_lob, ordered_position = self.kevin_market_take(product, 
                                                                                                               best_ask_coupon, 
            ...
            if sell_available_position_coupon > 0:
                ...
                order_coupon, sell_available_position_coupon, estimated_traded_lob, ordered_position = self.kevin_market_take(product,
                                                                                                                best_bid_coupon,
                                                                                                                best_bid_coupon_amount,
                                                                                                                sell_available_position_coupon, -1,
                                                                                                                ordered_position, estimated_traded_lob)
                orders_coupon += order_coupon
        return orders_coupon, ordered_position, estimated_traded_lob
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths.keys():
            ...
            if product == "COCONUT_COUPON":
                ...
                orders_coupon, ordered_position, estimated_traded_lob = self.tongfei_BS_trade(product, state, ordered_position,
                                                                                       estimated_traded_lob, trade_coef, 
                                                                                       predicted_iv=predicted_iv, current_iv=ivs[0])
                result[product] = orders_coupon
            conversions = 0
