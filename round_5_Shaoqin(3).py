...
Rihana_position = {'GIFT_BASKET': 0, 'ROSES': 0}
...
class Trader:
    ...
    def rhianna_trade(self, state, product, traderDataOld, name='Rhianna'):
        ...
        if len(mkt_trade) > 0:
            ...
            mkt_trade = [trade for trade in mkt_trade if
                         (trade.buyer == name or trade.seller == name) and not self.assert_trade_equal(trade, stored_trade) and
                         trade.timestamp in [state.timestamp, state.timestamp - 100]]
        ...
        if len(mkt_trade) > 1:
            ...
            if flow > 0:
                ...
                buy_trade = [trade for trade in mkt_trade if trade.buyer == name]
                mkt_trade = [max(buy_trade, key=lambda x: x.quantity)]
            elif flow < 0:
                # we only want the largest sell trade
                sell_trade = [trade for trade in mkt_trade if trade.seller == name]
