...
K = 10000
T = 247 # 3 days as training data, 250-3
r = 0
EWMA_lambda = 0.94
MAX_ITERATIONS = 100
PRECISION = 1.0e-5
...
class Trader:
    ...
    def set_up_cached_trader_data(self, state, traderDataOld):
        ...
        coconut_midprice = self.calculate_mid_price(state, 'COCONUT')
        coconut_delta = self.delta_call(coconut_midprice, K, r, 0.010163610080792894, T)
        # cache formulation
        current_cache = [{'STARFRUIT': [star_midprice, star_standford_midprice, star_majority_vol, star_imbalance],
                          ...
                          'COCONUT': [coconut_midprice, coconut_delta]}]
    ...
    def tongfei_calculate_fair_price(self, product, state, ordered_position, estimated_traded_lob, latest_coconut_price):
        ...
        fair_price = self.Black_Scholes(latest_coconut_price, K, r, 0.010163610080792894, T, 'call')
        print(f"fair_price: {fair_price}, mid_price: {mid_price}")
        # one standard deviation
        if mid_price > fair_price + 5:
            # the price is considered overvalued
            return -1
        elif mid_price < fair_price - 5:
    ...
    def tongfei_BS_trade(self, product_list, state, ordered_position, estimated_traded_lob, trade_coef, previous_delta):
        ...
        best_bid_coconut, best_bid_coconut_amout, best_ask_coconut, best_ask_coconut_amout = self.get_best_bid_ask(product_list[0], estimated_traded_lob)
        ...
        delta = self.delta_call(mid_price_coconut, K, r, 0.010163610080792894, T)
        if trade_coef == 1:
            # we buy option (coupon), sell coconut
            if buy_available_position_coupon > 0:
                ...
                delta_hedge_amount = int(np.round(delta * min(np.abs(best_ask_coupon_amount), buy_available_position_coupon)))
            ...
            # we sell option (coupon), buy coconut
            if sell_available_position_coupon > 0:
                ...
                orders_coupon += order_coupon
                # we do delta hedge on new bought coupon
                delta_hedge_amount = int(np.round(delta * min(best_bid_coupon_amount, sell_available_position_coupon)))
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths.keys():
            if product == "COCONUT":
                latest_coconut_price = self.calculate_mid_price(state, product)

            if product == "COCONUT_COUPON":
                previous_deltas = self.extract_from_cache(traderDataNew, 'COCONUT', 1) # 0 is the current value, 1 is the previous value
                if len(previous_deltas) == 1:
                    previous_delta = previous_deltas[0]
                else:
                    previous_delta = previous_deltas[1]
                trade_coef = self.tongfei_calculate_fair_price(product, state, ordered_position, estimated_traded_lob, latest_coconut_price)
